...
export type IBetBallType = 'primitiva-number' | 'primitiva-reintegro' | 'euromillones-number' | 'euromillones-star';
...
export const betsReducer = createReducer(betsInitialState, (builder) => {
  ...
  builder.addCase(toggleBall, (state, action) => {
    switch (action.payload.type) {
      case 'primitiva-number': {
        state.primitiva.numbers = toggleSelection([...state.primitiva.numbers], action.payload.value, 6);
        break;
      }
      case 'primitiva-reintegro': {
        state.primitiva.reintegro = state.primitiva.reintegro === action.payload.value ? null : action.payload.value;
        break;
      }
      case 'euromillones-number': {
        state.euromillones.numbers = toggleSelection([...state.euromillones.numbers], action.payload.value, 5);
        break;
      }
      case 'euromillones-star': {
        state.euromillones.stars = toggleSelection([...state.euromillones.stars], action.payload.value, 2);
        break;
      }
      default:
  ...
  builder.addCase(setRandomBet, (state) => {
    if (state.activeGame === 'primitiva') {
      state.primitiva.numbers = pickRandomUnique(6, 1, 49).sort((a, b) => a - b);
      state.primitiva.reintegro = randomBetween(0, 9);
    } else {
      state.euromillones.numbers = pickRandomUnique(5, 1, 50).sort((a, b) => a - b);
      state.euromillones.stars = pickRandomUnique(2, 1, 12).sort((a, b) => a - b);
    }
  });
});
