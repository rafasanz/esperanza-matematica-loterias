...
export const Ball: FC<IBallProps> = ({ isStar, type, value }) => {
  ...
  const isActive = useAppSelector((state) => selectIsBallActive(state, { type, value }));

  return (
    <button
      onClick={() => dispatch(toggleBall({ type, value }))}
      className={cls(styles.Ball, isStar && styles.starBall, isActive && styles.active)}
      data-ball-type={type}
      data-ball-value={value}
    >
      <span className={styles.ballText}>{String(value).padStart(2, '0')}</span>
      {isStar && (
        <svg className={styles.starShape} viewBox='0 0 100 100' aria-hidden='true' focusable='false'>
